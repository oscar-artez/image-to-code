...
export default function Home() {
  ...
  const transformToCode = async (body: string) => {
    ...
    if (!res.ok || res.body == null) {
      setStep(STEPS.ERROR)
      throw new Error('Error al transformar la imagen a código')
    }
  ...
  return (
    <div className="grid grid-cols-[400px_1fr]">
      <aside className="flex flex-col justify-between min-h-screen p-4 bg-gray-900">
        ...
        <section></section>
        <footer>Desarrollado por Jartez</footer>
      </aside>
          ...
          {step === STEPS.PREVIEW && (
            <div className="rounded flex flex-col gap-4">
              <div
                ...
                className="w-full h h-full border-4 rounded border-gray-700  aspect-video"
              >
                <iframe srcDoc={html} className="w-full h h-full "></iframe>
              </div>
              <pre className="">
                <code>{html}</code>
              </pre>
